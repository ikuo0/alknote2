...
def delete_all_verify_tokens(cfg: Config) -> None:
    """ddbVerifyToken テーブルの全レコードを削除する"""
    client = make_ddb_client(cfg)
    resp = client.scan(TableName="ddbVerifyToken", ProjectionExpression="verify_token")
    items = resp.get("Items", [])
    for item in items:
        client.delete_item(
            TableName="ddbVerifyToken",
            Key={"verify_token": {"S": item["verify_token"]["S"]}},
        )


# ─────────────────────────────────────────────
# フィクスチャ: テスト開始前に全レコード削除
# ─────────────────────────────────────────────

@pytest.fixture(scope="session", autouse=True)
def cleanup_before_all_tests():
    cfg = get_config("local")
    delete_all_verify_tokens(cfg)
    yield
